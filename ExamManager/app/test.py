...
import logging
if os.path.exists('./Exammanager/log.txt'):
        os.remove('./Exammanager/log.txt')
...
def run():
    ...
    # eTest = models.Student.query.all()
    # for u in eTest:
    #     print(u.student_dateOfBirth)
    db.session.commit()
...
def test_register_teacher():
    ...
    # logfile to be added
        ...
        if failed ==False:
            logger.debug(f"GO TO TEACHER PAGE \n {examiner.examiner_name}")
        else :
            logger.debug(f"GO TO TEACHER PAGE FAILED \n {examiner.examiner_name}")
...
def test_register_teacher_same_email():
    failed = False
    try :
        examiner = models.Examiner(examiner_name="John MCc  ", examiner_surname="Lauren",
                    examiner_email="Johnlaure5@gmail.com", examiner_password="jjjjj")
        db.session.add(examiner)
        db.session.commit()
    except Exception as e :
...
def test_register_student():
    failed = False
    try :
        student =  models.Student(student_name="Piotr", student_surname="Krzystanek",
                            student_email="PiotrKrzystanek0@gmail.com", student_password="kkkkk")
        db.session.add(student)
        db.session.commit()
    ...
    # logfile to be added
        ...
        if failed ==False:
            logger.debug(f"GO TO STUDENT PAGE \n {student.student_name}")
        else :
            logger.debug(f"GO TO STUDENT PAGE FAILED \n {student.student_name}")
...
def test_register_student_same_email():
    failed = False
    try :
        student =  models.Student(student_name="Piotrek", student_surname="Krzystanek",
